...
class DetailedLossLogger:
    ...
    def get_statistics(self):
        ...
        try:
            ...
            print(f"Error reading detailed metrics: {e}")
            return {}
        ...
        stats = {}
        loss_fields = ['loss_start_y', 'loss_start_x', 'loss_theta', 'loss_length',
                      'reg_xytl_loss', 'cls_loss', 'iou_loss', 
                      'loss_category', 'loss_attribute']
    ...
    def print_summary(self):
        ...
        print("\n" + "="*80)
        print("Detailed Loss Summary")
        print("="*80)
        print(f"\nRecords: {len(self._read_all_metrics())} iterations")
        
        # XYTL 分项损失
        print("\n【XYTL Regression Loss Components】")
        print(f"{'Loss':<15} | {'Min':>10} | {'Max':>10} | {'Mean':>10} | {'Last':>10}")
        print("-"*65)
        for field in ['loss_start_y', 'loss_start_x', 'loss_theta', 'loss_length']:
            if field in stats:
                s = stats[field]
                name = field.replace('loss_', '').replace('_', ' ').title()
                print(f"{name:<15} | {s['min']:>9.4f} | {s['max']:>9.4f} | {s['mean']:>9.4f} | {s['last']:>9.4f}")
        
        # 其他损失
        print("\n【Other Losses】")
        print(f"{'Loss':<15} | {'Min':>10} | {'Max':>10} | {'Mean':>10} | {'Last':>10}")
        print("-"*65)
        for field in ['reg_xytl_loss', 'cls_loss', 'iou_loss', 'loss_category', 'loss_attribute']:
            if field in stats:
                s = stats[field]
                name = field.replace('loss_', '').replace('_', ' ').title()
                print(f"{name:<15} | {s['min']:>9.4f} | {s['max']:>9.4f} | {s['mean']:>9.4f} | {s['last']:>9.4f}")
